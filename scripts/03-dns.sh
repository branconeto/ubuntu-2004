...
echo -e "Atualizando os arquivos de configuração do Bind DNS Server, aguarde..."
	...
	cp -v conf/dns/{sbn.local.hosts,192.168.1.rev} /var/lib/bind/ &>> $LOG
	...
	chown -v root:bind /var/lib/bind/{sbn.local.hosts,192.168.1.rev} &>> $LOG
...
sleep 5
#
echo -e "Editando o arquivo de configuração sbn.local.hosts, pressione <Enter> para continuar."
	# opção do comando: &>> (redirecionar a saída padrão)
	# opção do comando read: -s (Do not echo keystrokes)
	read -s
	vim /var/lib/bind/sbn.local.hosts
	named-checkzone $DOMAIN /var/lib/bind/sbn.local.hosts &>> $LOG
echo -e "Arquivo editado com sucesso!!!, continuando com o script...\n"
sleep 5
#
